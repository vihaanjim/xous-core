...
const LABYRINTH_SERVER_NAME: &'static str = "User app 'labyrinth'";

/// Opcodes for the application main loop
#[derive(Debug, num_derive::FromPrimitive, num_derive::ToPrimitive)]
pub(crate) enum AppOp {
    /// pumps the state of the labyrinth
    Pump,
    /// redraw our screen
    Redraw,
...
const LABYRINTH_UPDATE_RATE_MS: usize = 50;
...
fn xmain() -> ! {
    ...
    let sid = xns.register_name(LABYRINTH_SERVER_NAME, None).expect("can't register server");
    log::trace!("registered with NS -- {:?}", sid);

    // create the labyrinth object
    let mut labyrinth = Labyrinth::new(sid);
    ...
    labyrinth::labyrinth_pump_thread(xous::connect(sid).unwrap(), pump_sid);
